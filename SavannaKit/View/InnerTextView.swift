...
import UIKit
#endif
protocol InnerTextViewDelegate: class { func didUpdateCursorFloatingState() }

class InnerTextView: PlatformTextView {
  weak var innerDelegate: InnerTextViewDelegate?
  var theme: SyntaxColorTheme?
  var cachedParagraphs: [Paragraph]?
  func invalidateCachedParagraphs() { cachedParagraphs = nil }

  func hideGutter() { gutterWidth = theme?.gutterStyle.minimumWidth ?? 0.0 }
  ...
  override public func draw(_ rect: CGRect) {
    ...
    if theme.lineNumbersStyle == nil {
      ...
      let gutterRect = CGRect(
        x: 0,
        y: rect.minY,
        width: textView.gutterWidth,
        height: rect.height
      )
      let path = PlatformBezierPath(rect: gutterRect)
      path.fill()
    } else {
  ...
  override func caretRect(for position: UITextPosition) -> CGRect {
    ...
    guard let theme = theme else { return superRect }
