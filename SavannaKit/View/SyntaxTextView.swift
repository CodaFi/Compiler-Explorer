...
open class SyntaxTextView: PlatformView {
  struct DelegateCallbacks {
    let didChangeText: (_ syntaxTextView: SyntaxTextView) -> Void
    let didChangeSelectedRange: (_ syntaxTextView: SyntaxTextView, _ selectedRange: NSRange) -> Void
    let textViewDidBeginEditing: (_ syntaxTextView: SyntaxTextView) -> Void
  }

  var previousSelectedRange: NSRange?
  private var textViewSelectedRangeObserver: NSKeyValueObservation?
  let textView: InnerTextView
  public var contentTextView: PlatformTextView { return textView }

  let callbacks: DelegateCallbacks
  var ignoreSelectionChange = false
  #if os(macOS)
  let wrapperView = PlatformTextViewWrapperView()
  #endif
  #if os(iOS)
  ...
  open override var tintColor: UIColor! { didSet {}
  }
  ...
  public init(
    frame: CGRect,
    lexer: Lexer,
    didChangeText: @escaping (_ syntaxTextView: SyntaxTextView) -> Void
  ) {
    self.callbacks = DelegateCallbacks(
  ...
  private func setup() {
    ...
    textViewSelectedRangeObserver = contentTextView.observe(\UITextView.selectedTextRange) {
      [weak self] (textView, value) in
      if let `self` = self {
  ...
  #if os(macOS)
  open override func viewDidMoveToSuperview() { super.viewDidMoveToSuperview() }
  @objc
  func didScroll(_ notification: Notification) { wrapperView.setNeedsDisplay(wrapperView.bounds) }

  #endif
  // MARK: -
  #if os(iOS)
  override open var isFirstResponder: Bool { return textView.isFirstResponder }
  ...
  public var lexer: Lexer { didSet { self.didUpdateText() }
  }
  ...
  #if os(iOS)
  public func scrollViewDidScroll(_ scrollView: UIScrollView) { self.textView.setNeedsDisplay() }
  override
  ...
  var themeInfo: ThemeInfo? {
    if let cached = cachedThemeInfo { return cached }
  ...
  func invalidateCachedTokens() { cachedTokens = nil }

  func colorTextView(_ lexer: Lexer) {
  ...
  func updateAttributes(textStorage: NSTextStorage, cachedTokens: [CachedToken], source: String) {
    ...
    textStorage.enumerateAttribute(.editorPlaceholder, in: fullRange, options: []) {
      (value, range, stop) in
      if let state = value as? EditorPlaceholderState {
        ...
        if newState != state { rangesToUpdate.append((range, newState)) }
    ...
    if didBeginEditing { textStorage.endEditing() }
  ...
  func createAttributes(
    ...
    for (attr, value) in theme.globalAttributes() { attributes[attr] = value }
    ...
    for cachedToken in cachedTokens {
      ...
      if token.isEditorPlaceholder {
        ...
        textStorage.addAttributes(
          [.foregroundColor: PlatformColor.clear, .font: PlatformFont.systemFont(ofSize: 0.01)],
          range: startRange
        )
        textStorage.addAttributes(
          [.foregroundColor: PlatformColor.clear, .font: PlatformFont.systemFont(ofSize: 0.01)],
