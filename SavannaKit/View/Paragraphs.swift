...
extension PlatformTextView {
  func paragraphRectForRange(range: NSRange) -> CGRect {
    ...
    if sectionRect.origin.x == 0 { sectionRect.size.height -= 22 }
...
func generateParagraphs(for textView: InnerTextView, flipRects: Bool = false) -> [Paragraph] {
  ...
  var i = 0
  (textView.text as NSString).enumerateSubstrings(in: range, options: [.byParagraphs]) {
    (paragraphContent, paragraphRange, enclosingRange, stop) in
    i += 1
  ...
  if textView.text.isEmpty || textView.text.hasSuffix("\n") {
    ...
    if let last = paragraphs.last {
      ...
      rect = CGRect(
        x: 0,
        y: last.rect.origin.y + last.rect.height + 2,
        width: gutterWidth,
...
func offsetParagraphs(_ paragraphs: [Paragraph], for textView: InnerTextView, yOffset: CGFloat = 0)
  -> [Paragraph]
{
