...
extension NSAttributedString.Key {
  public static let editorPlaceholder = NSAttributedString.Key("editorPlaceholder")
}
...
class SyntaxTextViewLayoutManager: NSLayoutManager {
  override func drawGlyphs(forGlyphRange glyphsToShow: NSRange, at origin: CGPoint) {
    ...
    for (rect, state) in placeholders {
      ...
      switch state {
      case .active: color = PlatformColor.white.withAlphaComponent(0.8)
      case .inactive: color = .darkGray
      }

      color.setFill()
      let radius: CGFloat = 4.0
      #if os(macOS)
        let path = PlatformBezierPath(roundedRect: rect, xRadius: radius, yRadius: radius)
      #else
        let path = PlatformBezierPath(roundedRect: rect, cornerRadius: radius)
      #endif
      path.fill()
    }
