...
public class UniversalTheme<TokenType: UniversalToken>: SyntaxColorTheme {
  ...
  public var gutterStyle: GutterStyle {
    ...
    if appearanceName == .darkAqua {
      return GutterStyle(backgroundColor: PlatformColor(red: 21/255.0, green: 22/255, blue: 31/255, alpha: 1.0), minimumWidth: 32)
    } else if appearanceName == .aqua {
      return GutterStyle(backgroundColor: PlatformColor(red: 236/255.0, green: 236/255, blue: 236/255, alpha: 1.0), minimumWidth: 32)
    } else {
      return GutterStyle(backgroundColor: PlatformColor(red: 21/255.0, green: 22/255, blue: 31/255, alpha: 1.0), minimumWidth: 32)
    }
    #else
  ...
  public func attributes(for token: Token) -> [NSAttributedString.Key: Any] {
    ...
    guard let exactToken = token as? TokenType else {
      return [:]
    }
